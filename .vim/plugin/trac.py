...
class TracTicket(TracRPC):
    ...
    def parse_changelog_datetime(self, changelog):
        """ Converts the strange DateTime object the changelog is returned as
        into an authentic datetime object:
            
        >>> tt = tracTicket('foo')
        >>> weird_DateTime_obj = 20080922T12:42:28
        >>> tt.parse_changelog_datetime(weird_DateTime_obj)
        datetime.datetime(2008, 9, 22, 12, 42, 28)
        
        """
        datestr, time = str(changelog).split('T')
        year, month, day = datestr[:4], datestr[4:6], datestr[6:8]
        hour, minute, second = time.split(':')
        return datetime(*(int(i) for i in (year, month, day, hour, minute, second)))
    def setServer (self, url):
        self.server = xmlrpclib.ServerProxy(url)
        self.getOptions()
    def getOptions (self): 
    ...
    def getAllTickets(self,owner, b_use_cache = False):
        ...
        for ticket in tickets:
            ...
            if ticket[3]["status"] != "closed" and self.filter.check(ticket):
                ...
                component =  ticket[3].get("component", '')
                if (component == ''):
    ...
    def getAllTicketsSummary(self,owner, b_use_cache = False):
        ...
        for ticket in tickets:
            ...
            if ticket[3]["status"] != "closed" and self.filter.check(ticket):
                ...
                str_ticket += ". || " + ticket[3]["summary"][0:MAX_TICKET_SUMMARY]  
                if len(ticket[3]["summary"]) > MAX_TICKET_SUMMARY:
                    str_ticket += "..."
    ...
    def getTicket(self, id):
        ...
        str_ticket += "*        Type: " + ticket[3]["type"]      + "\n"
        str_ticket += "*    Priority: " + ticket[3]["priority"]  + "\n"
        str_ticket += "*   Component: " + ticket[3].get("component", '') + "\n"
        str_ticket += "*   Milestone: " + ticket[3].get("milestone", '')+ "\n"
        str_ticket += "*     Version: " + ticket[3].get("version", '') + "\n"
        ...
        for change in ticket_changelog:
            if change[4] != '':
                dt = self.parse_changelog_datetime(change[0])
...
class TracTicketSort:
    ...
    def sort(self,tickets):
        """ Ticket sorting -
        TODO: should probably use python sort
        trac.ticket.a_option:
        [
            ['Friends and family launch', 'teacher functionality working',
             'v1.0', 'v1.1', 'v1.2', 'v1.3', 'v1.4', 'v1.5', 'v1.6', 
             'v1.7', 'v1.8', 'v1.9'], 
            ['defect', 'enhancement', 'task'], 
            ['accepted', 'assigned', 'closed', 'new', 'reopened'], 
            ['fixed', 'review_required', 'invalid', 'wontfix', 'duplicate',
             'worksforme'], 
            ['blocker', 'critical', 'major', 'minor', 'trivial'], 
            [], 
            [], <xmlrpclib.MultiCallIterator instance at 0x579aad0>
        ]  

        {'status':      'new',
         'description': 'New classes as per UML\n\nProduct class is new and
                         requires removal of most of the classes in 
                         llcom.products.models.products module and conversion
                         of all the data.\n\nScheduledProduct class will need
                         changes to move it inline with the new Product
                         class.\n',
         'reporter':    'lee',
         'cc':          '',
         'type':        'task',
         'summary':     'Add new classes and functionality for courses',
         'priority':    'blocker',
         'owner':       '',
         'milestone':   'v1.4',
         'keywords':    'courses',
        ...
        if self.sortby == 'priority':
            return tickets
        elif self.sortby == 'owner':
            sorted_tickets = []
            for owner in TICKET_OWNERS:
                for ticket in tickets:
                    if ticket[3]['owner'] == owner:
                        sorted_tickets.append(ticket)
                for ticket in tickets:
                    if ticket[3]['owner'] not in TICKET_OWNERS and \
                       ticket[3]['owner'] != '':
                        sorted_tickets.append(ticket)
                #append tickets without owners
                for ticket in tickets:
                    if ticket[3]['owner'] == '':
                        sorted_tickets.append(ticket)
